...
//#include "Randomize.hh" // MS 2018/12/09 for PMT Random Shift
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructCylinder()
{
  ...
  G4cout << "vessel_cyl_height = " << vessel_cyl_height << G4endl;
  G4cout << "vessel_radius = " << vessel_radius << G4endl;
  G4cout << "vessel_cyl_height + vessel_radius = " << vessel_cyl_height + vessel_radius << G4endl;
  G4cout << "WCIDRadius = " << WCIDRadius << G4endl;
  G4cout << "WCPMTExposeHeight = " << WCPMTExposeHeight << G4endl;
  ...
  G4cout << "innerAnnulusRadius = " << innerAnnulusRadius << G4endl;
  ...
  G4cout << "WCLength = " << WCLength << G4endl;
  G4cout << "WCRadius = " << WCRadius << G4endl;
...
// These lines add color to the blacksheet in the extratower. If using RayTracer, comment the first chunk and use the second. The Blacksheet should be green.
  ...
  if(WCTopVeto){
	...
	G4cout << "--- MC_Check Top Veto Volume ---" << G4endl;
...
If used here, uncomment the SetVisAttributes(WClogic) line, and comment out the SetVisAttributes(G4VisAttributes::Invisible) line.*/
  ...
  if(WCTopVeto){
	...
	G4cout << "--- MC_Check Top Veto PMTs ---" << G4endl;
  ...
  G4cout << "--- MS 2018/12/09 Parameter Check ---" << G4endl;
  G4cout << "MS WCIDHeight = " << WCIDHeight << " mm" << G4endl;
  G4cout << "MS WCBarrelPMTOffset = " << WCBarrelPMTOffset << " mm" << G4endl;
  G4double barrelCellWidth = 2.*WCIDRadius*tan(dPhi/2.);
  G4double horizontalSpacing   = barrelCellWidth/WCPMTperCellHorizontal;
  G4cout << "MS horizontalSpacing = " << horizontalSpacing << " mm" << G4endl;
  G4cout << "MS barrelCellWidth = " << barrelCellWidth << " mm" << G4endl;
  G4cout << "MS WCPMTperCellHorizontal = " << WCPMTperCellHorizontal << G4endl;
  G4double verticalSpacing     = barrelCellHeight/WCPMTperCellVertical;
  G4cout << "MS verticalSpacing = " << verticalSpacing << " mm" << G4endl;
  G4cout << "MS barrelCellHeight = " << barrelCellHeight << " mm" << G4endl;
  G4cout << "MS WCPMTperCellVertical = " << WCPMTperCellVertical << G4endl;

  if(placeBarrelPMTs){

	G4cout << "--- MS_Check Barrel PMTs Placement ---" << G4endl;
  ...
  for(G4double i = 0; i < WCPMTperCellHorizontal; i++){
    for(G4double j = 0; j < WCPMTperCellVertical; j++){
	  ...
	  // If PMT shifted (MS 2018/12/09) ********************************
	  /*
	  G4double random_horizontal = G4UniformRand()*60-30;
	  G4double random_vertical = G4UniformRand()*80-40;
	  G4ThreeVector PMTPosition =  G4ThreeVector(WCIDRadius,
						 -barrelCellWidth/2.+(i+0.5)*horizontalSpacing+random_horizontal,
						 -barrelCellHeight/2.+(j+0.5)*verticalSpacing+random_vertical);
	  */
	  // ***************************************************************
...
G4LogicalVolume* WCSimDetectorConstruction::ConstructCaps(G4int zflip)
{
  ...
  G4cout << "capAssemblyHeight = " << capAssemblyHeight << G4endl;
  ...
  //---------------------------------------------------
  G4cout << "barrelCellHeight = " << barrelCellHeight << G4endl;
  G4double borderAnnulusZ[3] = {(-barrelCellHeight/2.-(WCIDRadius-innerAnnulusRadius))*zflip, 
								-barrelCellHeight/2.*zflip,
								barrelCellHeight/2.*zflip};
  G4cout << "borderAnnulusZ = (" << borderAnnulusZ[0] << ", " << borderAnnulusZ[1] << ", " << borderAnnulusZ[2] << ")" << G4endl;
  G4double borderAnnulusRmin[3] = { WCIDRadius, innerAnnulusRadius, innerAnnulusRadius};
  G4cout << "borderAnnulusRmin = (" << borderAnnulusRmin[0] << ", " << borderAnnulusRmin[1] << ", " << borderAnnulusRmin[2] << ")" << G4endl;
  G4double borderAnnulusRmax[3] = {outerAnnulusRadius, outerAnnulusRadius,outerAnnulusRadius};
  G4cout << "borderAnnulusRmax = (" << borderAnnulusRmax[0] << ", " << borderAnnulusRmax[1] << ", " << borderAnnulusRmax[2] << ")" << G4endl;
  G4Polyhedra* solidWCBarrelBorderRing = new G4Polyhedra("WCBarrelBorderRing",
                                                   ...
                                                   borderAnnulusRmin,
                                                   borderAnnulusRmax);
  G4cout << "solidWCBarrelBorderRing = " << solidWCBarrelBorderRing << G4endl;
  G4LogicalVolume* logicWCBarrelBorderRing =
    new G4LogicalVolume(solidWCBarrelBorderRing,
                        G4Material::GetMaterial(water),
                        "WCBarrelRing",
                        0,0,0);
  G4cout << "logicWCBarrelBorderRing = " << logicWCBarrelBorderRing << G4endl;
  ...
  G4VPhysicalVolume* physiWCBarrelBorderRing =
    new G4PVPlacement(0,
                  ...
                  false, 0,
				  checkOverlaps);
  G4cout << "physiWCBarrelBorderRing = " << physiWCBarrelBorderRing << G4endl;
...
// used for OGLSX
 ...
 // -----------------------------------------------------------
  ...
  G4cout << "zflip = " << zflip << G4endl;
  G4cout << "WCBlackSheetThickness = " << WCBlackSheetThickness << G4endl;
  ...
  G4cout << "capZ = (" << capZ[0] << ", " << capZ[1] << ", " << capZ[2] << ", " << capZ[3] << ")" << G4endl;

  G4double capRmin[4] = {  0. , 0., 0., 0.} ;
  G4double capRmax[4] = {outerAnnulusRadius, outerAnnulusRadius,  WCIDRadius, innerAnnulusRadius};
  G4cout << "capRmax = (" << capRmax[0] << ", " << capRmax[1] << ", " << capRmax[2] << ", " << capRmax[3] << ")" << G4endl;
  G4cout << "WCBarrelRingNPhi = " << WCBarrelRingNPhi << G4endl;
  G4cout << "WCPMTperCellHorizontal = " << WCPMTperCellHorizontal << G4endl;
  G4cout << "WCBarrelNumPMTHorizontal = " << WCBarrelNumPMTHorizontal << G4endl;
  G4cout << "totalAngle = " << totalAngle << G4endl;
  G4VSolid* solidWCCap;
  if(WCBarrelRingNPhi*WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal){
	G4cout << "-- MS_Check1 --" << G4endl;
    solidWCCap
...
// used for RayTracer (makes the caps blacksheet yellow)
 ...
 // If using RayTracer and want to view the detector without caps, comment out the top and bottom PMT's
  ...
  G4cout << "MS-CHECK Rotation" << G4endl;
  G4cout << "orientation = " << orientation << G4endl;
  G4cout << "PERPENDICULAR = " << PERPENDICULAR << G4endl;
  G4cout << "VERTICAL = " << VERTICAL << G4endl;
  G4cout << "HORIZONTAL = " << HORIZONTAL << G4endl;
  
  //if mPMT: perp to wall
  if(orientation == PERPENDICULAR){
	G4cout << "MS-CHECK PERPENDICULAR-1" << G4endl;
	if(zflip==-1){
	  G4cout << "MS-CHECK PERPENDICULAR-2" << G4endl;
  ...
  G4cout << "MS CapNCell = " << CapNCell << G4endl;
  G4cout << "MS WCCapEdgeLimit = " << WCCapEdgeLimit << G4endl;
  G4cout << "MS WCCapPMTSpacing = " << WCCapPMTSpacing << G4endl;
  G4cout << "MS WCPMTRadius = " << WCPMTRadius << G4endl;
  for ( int i = -CapNCell ; i <  CapNCell; i++) {
    for (int j = -CapNCell ; j <  CapNCell; j++)   {
	  ...
	  // For Test WCD  M.Shinoki 2018/11/29
	  xoffset = i*WCCapPMTSpacing;
	  yoffset = j*WCCapPMTSpacing;

	  //G4cout << "(xoffset, yoffset) = " << xoffset << ", " << yoffset << ")" << G4endl;
	  ...
	  //G4cout << "sqrt(xoffset*xoffset + yoffset*yoffset) + WCPMTRadius = " << sqrt(xoffset*xoffset + yoffset*yoffset) + WCPMTRadius << G4endl;
	  if ((sqrt(xoffset*xoffset + yoffset*yoffset) + WCPMTRadius) < WCCapEdgeLimit) 
		...
		// &&  ((sqrt(xoffset*xoffset + yoffset*yoffset) + WCPMTRadius) > (WCCapEdgeLimit-100)) ) 
		{
		  ...
		  // If PMTs are shifted **************************
		  /*
		  if ((abs(xoffset)<3000) && (abs(yoffset)<3000)){
			G4double random_xoffset = G4UniformRand()*60-30;
			G4double random_yoffset = G4UniformRand()*60-30;
			xoffset = xoffset + random_xoffset;
			yoffset = yoffset + random_yoffset;
		  }
		  cellpos = G4ThreeVector(xoffset, yoffset, 0);
		  */
		  // **********************************************

		  //G4cout << "MS WCCapPMTRotation = " << WCCapPMTRotation << G4endl; // 2019/02/20 M.Shinoki
	G4VPhysicalVolume* physiCapPMT =
 ...
 // logicWCPMT->GetDaughter(0),physiCapPMT is the glass face. If you add more 
  ...
  // 2019/02/20 M.Shinoki
  G4cout << "MS-CHECK Barrel orientation = " << orientation << G4endl;
  G4cout << "MS-CHECK Barrel PERPENDICULAR = " << PERPENDICULAR << G4endl;
  G4cout << "MS-CHECK Barrel VERTICAL = " << VERTICAL << G4endl;
  G4cout << "MS-CHECK Barrel HORIZONTAL = " << HORIZONTAL << G4endl;
  
  if(orientation == PERPENDICULAR){
	WCPMTRotation->rotateY(90.*deg); //if mPMT: perp to wall // default
  }
  else if(orientation == VERTICAL)
	WCPMTRotation->rotateY(0.*deg); //if mPMT: vertical/aligned to wall
  else if(orientation == HORIZONTAL)
